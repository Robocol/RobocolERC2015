...
/*!
**     @resumen
**          Convierte un uint16_t a una cadena de caracteres (string)
**     @param
**          i 	    	   	- Número a convertir
**     @preturn
**          				- Apuntador a la primera posición del array.
...
gpio_st g_write_file(char* ruta, char* buff, uint8_t len ){
	...
	if(write(fd,buff,len)<0)
	{
		...
		printf("Error al escribir %s en %s\n",buff,ruta );
		perror("Descripción");
		return GPIO_ERROR;
	}
...
*/
/* ===================================================================*/
gpio_st g_read_file(char* ruta, char* buff, uint8_t len ){
	//Se abre el archivo para incluir el GPIO en sysfs
	int fd;
	fd=open(ruta,O_WRONLY);
