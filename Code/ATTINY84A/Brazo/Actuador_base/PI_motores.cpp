...
int main(void) {
	...
	while (1) {
		...
		case AUTO_EST:
			...
			if((0x04 & PINA)>>2){//El actuador sube: PID en cascada con corriente (lazo interno)
				//gpio_put(LED_PIN,1);	
				direccion=1;
				...
				direccion=4;	
			}else{//El actuador baja: ON/OFF en cascada con corriente (lazo interno)
				//gpio_put(LED_PIN,0);
				direccion=5;	
